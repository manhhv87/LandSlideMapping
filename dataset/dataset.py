...
class LandslideDataSet(data.Dataset):
    def __init__(self, data_dir, list_path, set_mask='masked', transform=None):
        ...
        self.mean = [-0.4914, -0.3074, -0.1277, -0.0625, 0.0439, 0.0803,
                     0.0644, 0.0802, 0.3000, 0.4082, 0.0823, 0.0516, 0.3338, 0.7819]
        self.std = [0.9325, 0.8775, 0.8860, 0.8869, 0.8857, 0.8418,
        ...
        if set_mask == 'masked':
            for name in self.img_ids:
                ...
                mask_file = data_dir + \
                    name.replace('img', 'mask').replace('image', 'mask')
                self.files.append({
                    'img': img_file,
                    'masked': mask_file,
                    'name': name
                })
        if set_mask == 'unmasked':
            for name in self.img_ids:
                img_file = data_dir + name
                self.files.append({
                    'img': img_file,
...
if __name__ == '__main__':
    train_dataset = LandslideDataSet(
        data_dir='./data/', list_path='./dataset/train.txt')
