...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, groups=1, bias=False):
        ...
        self.conv1 = nn.Conv2d(
            in_planes, out_planes, kernel_size, stride, padding, groups=groups, bias=bias)
        self.bn1 = nn.BatchNorm2d(out_planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(
            out_planes, out_planes, kernel_size, 1, padding, groups=groups, bias=bias)
...
class LinkNet(nn.Module):
    ...
    def __init__(self, in_ch=3, n_classes=21):
        ...
        base = resnet.resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)

        # Creating new Conv2d layer
        base.conv1 = nn.Conv2d(
            in_ch, 64, 7, stride=2, padding=3, bias=False)
