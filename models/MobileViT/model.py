...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, in_ch=3, n_classes: int = 1000):
        super().__init__()
        ...
        self.classifier.add_module(name="fc", module=nn.Linear(
            in_features=exp_channels, out_features=n_classes))
...
def mobile_vit_xx_small(n_classes: int = 1000):
    ...
    m = MobileViT(config, in_ch=14, n_classes=n_classes)
    return m


def mobile_vit_x_small(n_classes: int = 1000):
    # pretrain weight link
...
def mobile_vit_small(in_ch=14, n_classes: int = 1000):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_s.pt
    config = get_config("small")
    m = MobileViT(config, in_ch=in_ch, n_classes=n_classes)
    return m
