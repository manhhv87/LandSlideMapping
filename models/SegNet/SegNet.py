...
class SegNet(BaseModel):
    def __init__(self, n_classes, in_ch, pretrained=False, freeze_bn=False, freeze_backbone=False, **_):
        ...
        if in_ch != 3:
            encoder[0] = nn.Conv2d(
                in_ch, 64, kernel_size=3, stride=1, padding=1)
        ...
        self.stage5_decoder = nn.Sequential(*decoder[33:],
                                            nn.Conv2d(
                                                64, n_classes, kernel_size=3, stride=1, padding=1)
                                            )
        ...
        if freeze_bn:
            self.freeze_bn()
        if freeze_backbone:
            self.set_trainable([self.stage1_encoder, self.stage2_encoder,
...
class SegResNet(BaseModel):
    def __init__(self, n_classes, in_ch, pretrained=False, freeze_bn=False, freeze_backbone=True, **_):
        ...
        if freeze_bn:
            self.freeze_bn()
        if freeze_backbone:
            self.set_trainable([self.first_conv, self.encoder], False)
